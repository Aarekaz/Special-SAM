...
Table~\ref{tab:comparison} places decoder-only adaptation in the broader COD landscape, but should be interpreted as contextual rather than head-to-head. Our method is evaluated in SAM's standard prompt-based setting, whereas dedicated COD methods are fully automatic. Under this protocol, decoder-only adaptation still recovers substantial performance with no new decoder parameters and 90-minute training, although it does not surpass encoder-adaptation methods (SAM-Adapter, COMPrompter) on $S_\alpha$ or MAE. This is consistent with our broader conclusion that encoder- and decoder-side specialization are complementary.
...
A natural concern is whether specializing for camouflage degrades performance on ordinary images. Figure~\ref{fig:coco} provides a qualitative sanity check on random COCO val2017 images using a fixed image-center prompt (no ground truth). Our decoder does not exhibit obvious catastrophic failure on these non-camouflage scenes. In several cases it produces \emph{tighter} segmentations than base SAM, which over-segments background regions when the center prompt is ambiguous. A quantitative study of general-image performance is left for future work.
