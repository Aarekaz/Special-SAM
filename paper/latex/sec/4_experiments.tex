...
Table~\ref{tab:comparison} contextualizes where decoder-only adaptation falls in the COD performance landscape. Despite using oracle GT-derived prompts (an advantage over fully automatic methods), our decoder-only approach does not surpass encoder-adaptation methods (SAM-Adapter, COMPrompter) on $S_\alpha$ or MAE. This is consistent with our thesis: encoder and decoder adaptation address complementary bottlenecks, and decoder-only fine-tuning---while efficient---does not replace encoder-side modification. The value of our approach lies in its simplicity (no new parameters, 90-min training) and the insight that decoder specialization alone recovers substantial performance.
...
Despite being trained exclusively on COD10K, the specialized decoder generalizes consistently across both held-out datasets. On CAMO, mIoU improves from 0.489 to 0.677 (+18.8 points); on NC4K (4,121 images), from 0.567 to 0.752 (+18.5 points). The improvement pattern---and the LLPM's additional gain---replicates without any retraining, confirming that decoder specialization learns transferable camouflage cues rather than dataset-specific artifacts.
...
\caption{\textbf{Qualitative results on regular COCO images} (fixed image-center prompt, no ground truth used). Camouflage specialization does not degrade general segmentation: our decoder produces tighter, more precise masks. Base SAM over-segments in several cases (rows 3 and 6), while our decoder remains conservative.}
\label{fig:coco}
\end{figure}

A natural concern is whether specializing for camouflage degrades performance on ordinary images. Figure~\ref{fig:coco} shows qualitative results on random COCO val2017 images using a fixed image-center prompt (no ground truth). Our decoder does not exhibit catastrophic failure on non-camouflage scenes. In several cases it produces \emph{tighter} segmentations than base SAM, which over-segments background regions when the center prompt is ambiguous. This is consistent with the decoder having learned more precise boundary delineation.
