...
We hypothesize that, for \textbf{promptable camouflaged object detection}, decoder adaptation is the main bottleneck: SAM's ViT-H encoder, pre-trained on 11M images, may already capture the low-level texture gradients that distinguish camouflaged foreground from background, but the decoder, trained on general segmentation, does not reliably map these signals to object boundaries. If this is true, fine-tuning only the lightweight 4M-parameter mask decoder should suffice.
