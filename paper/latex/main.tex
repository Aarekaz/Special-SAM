...
\begin{enumerate}
    \item A decoder-only fine-tuning pipeline for SAM that achieves +14.14 point mIoU improvement on COD10K with no architectural changes, evaluated on the full camouflaged test set (2,026 images).
    \item A prompt robustness analysis showing that decoder specialization transfers across all four tested prompt types, with the largest gains on the hardest prompts (+219.8\% relative on edge prompts).
...
Training with cosine annealing produced stable, monotonic loss decrease from 0.1417 (epoch 1) to 0.0717 (epoch 15), a 49.4\% total reduction. The final loss indicates healthy generalization without overfitting.
...
\midrule
Base SAM & Center-of-Mass (1pt) & 0.572 & 0.655 & 0.743 & 0.830 & 0.706 & 0.068 & 0.502 \\
...
Base SAM & Multi-Pt Random (3pt) & 0.730 & 0.817 & 0.831 & 0.930 & 0.832 & 0.043 & 0.652 \\
\midrule
\textbf{Ours} & \textbf{Center-of-Mass (1pt)} & \textbf{0.714} & \textbf{0.798} & \textbf{0.835} & \textbf{0.933} & \textbf{0.816} & \textbf{0.031} & \textbf{0.673} \\
\textbf{Ours} & \textbf{Edge (1pt)} & \textbf{0.656} & \textbf{0.742} & \textbf{0.790} & \textbf{0.909} & \textbf{0.732} & \textbf{0.036} & \textbf{0.631} \\
\textbf{Ours} & \textbf{Multi-Pt Grid (4pt)} & \textbf{0.723} & \textbf{0.807} & \textbf{0.843} & \textbf{0.939} & \textbf{0.826} & \textbf{0.028} & \textbf{0.681} \\
\textbf{Ours} & \textbf{Multi-Pt Random (3pt)} & \textbf{0.745} & \textbf{0.831} & \textbf{0.858} & \textbf{0.948} & \textbf{0.845} & \textbf{0.025} & \textbf{0.703} \\
\bottomrule
\end{tabular}
\end{table*}
...
The most striking result is the \textbf{uniformity of improvement across all prompt types}. While base SAM shows high sensitivity to prompt quality (mIoU ranges from 0.205 to 0.730), the specialized model is remarkably stable (0.656 to 0.745). The specialized model improves on \textbf{all four} strategies.
...
\midrule
Center-of-Mass & 0.572 & 0.714 & +0.141 & +24.7\% \\
Edge (Single) & 0.205 & 0.656 & +0.451 & \textbf{+219.8\%} \\
Multi-Pt Grid & 0.682 & 0.723 & +0.041 & +6.0\% \\
Multi-Pt Random & 0.730 & 0.745 & +0.015 & +2.0\% \\
\bottomrule
\end{tabular}
\end{table}

The \textbf{+219.8\% relative improvement on edge prompts} is the headline finding. Edge prompts are the hardest because they place the prompt point at the ambiguous boundary between camouflaged object and background. Base SAM produces nearly random masks (20.5\% mIoU) from edge prompts, while our specialized decoder achieves 65.6\%---comparable to its performance with easier prompt types. This suggests that decoder specialization fundamentally changes how the model resolves ambiguity at camouflaged boundaries.
...
\midrule
Amphibian & 124 & 0.680 & 0.780 & +14.8\% \\
Aquatic & 474 & 0.563 & 0.702 & +24.7\% \\
Flying & 714 & 0.610 & 0.745 & +22.0\% \\
Terrestrial & 699 & 0.519 & 0.675 & \textbf{+30.2\%} \\
Other & 15 & 0.694 & 0.868 & +25.0\% \\
\bottomrule
\end{tabular}
\end{table}

The largest improvement (+30.2\%) is on terrestrial animals, which exhibit the most challenging camouflage patterns (insects on bark, lizards on rocks). Aquatic animals also show substantial gains (+24.7\%), consistent with the difficulty of detecting fish and octopuses against coral/seabed textures.
...
\midrule
\textbf{Ours} (CoM) & --- & .835 & .933 & .816 & .031 \\
\textbf{Ours} (Grid) & --- & \textbf{.843} & \textbf{.939} & \textbf{.826} & \textbf{.028} \\
\bottomrule
\end{tabular}
\end{table}

Our decoder-only approach achieves $S_\alpha$ comparable to ZoomNet (0.843 vs.\ 0.838) and higher $E_\phi$ (0.939 vs.\ 0.911), despite using no architectural modifications. Methods with specialized adapters (SAM-Adapter, COMPrompter) achieve higher absolute performance by modifying the encoder.
...
% TODO: Add qualitative figure when generated from HPC job 5786513
% \begin{figure*}[t]
%     \centering
%     \includegraphics[width=\textwidth]{../figures/qualitative_comparison.png}
%     \caption{Qualitative comparison across difficulty levels.}
%     \label{fig:qualitative}
% \end{figure*}

Visual comparisons reveal that base SAM frequently segments only the most salient region near the prompt point, missing large portions of camouflaged objects. The specialized model produces masks that closely follow true object boundaries, even where the animal's coloration is nearly identical to the environment. On easier cases, both models perform comparably---specialization does not degrade performance.
...
We demonstrate that decoder-only fine-tuning of SAM ViT-H on COD10K produces a +14.14 point mIoU improvement on camouflaged object detection across the full 2,026-image test set, with a +219.8\% relative gain on edge prompts. The specialized model improves on all four tested prompt strategies. The key insight is that this improvement is \textbf{prompt-robust}: a decoder trained with mixed prompts generalizes to all tested strategies, including those never seen during training.
