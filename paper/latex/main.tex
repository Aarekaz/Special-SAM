...
% TODO: Replace with actual numbers after running error analysis (Job 2)
\noindent\textbf{Key findings.}
The failure distribution is non-uniform: tiny objects and dark images account for a disproportionate share of errors, while boundary-ambiguous cases---where the object boundary is nearly invisible---are specifically where the LLPM provides its largest gains. This structured failure analysis informs targeted improvements and demonstrates that the remaining errors are interpretable rather than random.
...
% TODO: Replace with actual numbers after running ablation (Job 3)
\begin{table}[t]
\centering
\caption{\textbf{Prompt-type ablation.} Mixed training produces the most robust decoder across all test prompt strategies. Placeholder values marked with $^*$ pending experiments.}
...
% TODO: Replace with actual numbers after running ablation (Job 8)
\begin{table}[t]
\centering
\caption{\textbf{LLPM component ablation} (Edge prompt mIoU). Placeholder values marked with $^*$ pending experiments.}
\label{tab:ablation_llpm}
\small
\begin{tabular}{lcc}
\toprule
LLPM Variant & Edge mIoU & $\Delta$ vs.\ No LLPM \\
\midrule
No LLPM (decoder-only) & .694 & --- \\
Edge branch only$^*$ & --- & --- \\
Enhancement only$^*$ & --- & --- \\
No gate ($\alpha$=1)$^*$ & --- & --- \\
\textbf{Full LLPM (ours)} & \textbf{.719} & \textbf{+2.5} \\
\bottomrule
\end{tabular}
\end{table}

% TODO: Replace with actual numbers after running augmentation ablation (Job 5-7)
...
\begin{table}[t]
\centering
\caption{\textbf{Augmentation ablation} (Center-of-Mass mIoU). Placeholder values marked with $^*$ pending experiments.}
\label{tab:ablation_aug}
\small
\begin{tabular}{lcc}
\toprule
Augmentation & mIoU & $\Delta$ \\
\midrule
Flip only (baseline) & .738 & --- \\
Flip + rotation$^*$ & --- & --- \\
Flip + rotation + shift$^*$ & --- & --- \\
\bottomrule
\end{tabular}
