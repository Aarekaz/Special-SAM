...
The two branches are fused via element-wise multiplication and a learnable gate $\alpha$ (initialized to $0$):
\begin{equation}
    x' = x + \alpha \cdot (h \odot e)
    \label{eq:llpm}
\end{equation}
where $\odot$ denotes the Hadamard product. The residual connection ensures that at initialization ($\alpha\!=\!0$), the module is an identity function and SAM's behavior is unchanged. As training progresses, $\alpha$ grows and the LLPM learns to inject boundary-enhancing perturbations into the input.
...
\begin{itemize}
    ...
    \item \textbf{Parameters:} $\sim$31K trainable parameters in the LLPM; the learnable fusion gate $\alpha$ is initialized to 0.
...
% TODO: This section will be populated once LLPM training on HPC completes.
% Include the following:
%   - Table comparing LLPM+decoder vs decoder-only across all prompt strategies
%   - Learned alpha value(s) and interpretation
%   - Key metric deltas: mIoU, Dice, S_alpha, E_phi, F_beta^w, MAE
%   - Per-category breakdown if space permits
%   - Ablation: LLPM-only (no decoder fine-tuning) vs decoder-only vs LLPM+decoder

Results for the LLPM variant are pending; this section will be updated once training completes.
